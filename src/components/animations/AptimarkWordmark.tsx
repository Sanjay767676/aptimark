...
// Simple local alternative to clsx/tailwind-merge since they are not installed in the workspace
  ...
  if (isComplete) {
    return (
      <AuroraText className={className}>
        {wordsToAnimate[wordsToAnimate.length - 1]}
      </AuroraText>
    )
  }
...
export default function AptimarkWordmark({ className = "" }: { className?: string }) {
  return (
    <TypingAnimation
      duration={150}
      showCursor={true}
